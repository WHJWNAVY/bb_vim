...
#define tot format_edit_status__tot
    ...
    ret = snprintf(status_buffer, trunc_at + 1,
                   ...
                   "%c %s%s %d/%d %d%%",
#endif
                   cmd_mode_indicator[cmd_mode & 3], (current_filename != NULL ? current_filename : "No file"),
#if ENABLE_FEATURE_VI_READONLY
...
static void undo_pop(void) {
    ...
    switch (undo_entry->u_type) {
        ...
        case UNDO_DEL_CHAIN:
            ...
            status_line("Undo [%d] %s %d chars at position %d", modified_count, "restored", undo_entry->length,
                        undo_entry->start);
            break;
        case UNDO_INS:
        case UNDO_INS_CHAIN:
            ...
            status_line("Undo [%d] %s %d chars at position %d", modified_count, "deleted", undo_entry->length,
                        undo_entry->start);
            break;
    }
    repeat = 0;
    ...
    *addr = -1;      // assume no addr
    if (*p == '.') { // the current line
        ...
        *addr = count_lines(text, q);
        ...
        if (*p == '/')
            p++;
        q = char_search(next_line(dot), last_search_pattern + 1, (FORWARD << 1) | FULL);
        if (q != NULL) {
            *addr = count_lines(text, q);
        }
    }
...
static void colon(char *buf) {
    ...
    if (strncmp(p, "write", cnt) == 0 || strncmp(p, "wq", cnt) == 0 || strncmp(p, "wn", cnt) == 0 ||
        ...
        if (cnt < 0) {
            ...
            if (p[0] == 'x' || p[1] == 'q' || p[1] == 'n' || p[1] == 'Q' || p[1] == 'N') {
                editing = 0;
            }
        }
        return;
    ...
    *buf1 = '\0';
    ...
    if (i == 0) { // :123CR goto line #123
        ...
        li = count_lines(text, end - 1);
        status_line("'%s'%s" IF_FEATURE_VI_READONLY("%s") " %uL, %uC", current_filename,
                    (size < 0 ? " [New file]" : ""),
                    IF_FEATURE_VI_READONLY(((readonly_mode) ? " [Readonly]" : ""), ) li, (int)(end - text));
        ...
        *flags++ = '\0'; // terminate "replace"
        ...
        status_line(BB_VER);
               ...
               || strncmp(cmd, "wq", i) == 0 || strncmp(cmd, "wn", i) == 0 || (cmd[0] == 'x' && !cmd[1])) {
        int size;
        // int forced = FALSE;
        ...
        if (l < 0) {
            ...
            if (l == size) {
                ...
                if (cmd[0] == 'x' || cmd[1] == 'q' || cmd[1] == 'n' || cmd[1] == 'Q' || cmd[1] == 'N') {
                    editing = 0;
                }
            }
        }
...
static int find_range(char **start, char **stop, char c) {
    ...
    if (strchr("cdy><", c)) {
        ...
        if (dot > text && ((isspace(dot[-1]) && !isspace(dot[0])) || (ispunct(dot[-1]) && !ispunct(dot[0])) ||
    ...
    switch (c) {
...
#endif
        {
            ...
            if (c1 != 27) {
                ...
                status_line("%s %u lines (%u chars) using [%c]", buf, cnt, (unsigned)strlen(reg[YDreg]), what_reg());
#endif
                end_cmd_q(); // stop adding to q
            }
            break;
...
static const char *const words[20] = {"this", "is",     "a",     "test",    "broadcast", "the",    "emergency",
...
static void crash_test() {
    ...
    if (msg[0]) {
        printf("\n\n%d: \'%c\' %s\n\n\n%s[Hit return to continue]%s", totalcmds, last_input_char, msg, ESC_BOLD_TEXT,
               ESC_NORM_TEXT);
        fflush_all();
    ...
    tim = time(NULL);
    if (tim >= (oldtim + 3)) {
        sprintf(status_buffer, "Tot=%d: M=%d N=%d I=%d D=%d Y=%d P=%d U=%d size=%d", totalcmds, M, N, I, D, Y, P, U,
                end - text + 1);
        oldtim = tim;
    }
}
...
void bb_show_usage(void) {
    fprintf(stderr, "\nUsage: %s [-c CMD] [-R] [FILE]...\n", applet_name);
    fprintf(stderr, "Edit FILE\n");
    fprintf(stderr, "-c CMD  Initial command to run ($EXINIT also available)\n");
